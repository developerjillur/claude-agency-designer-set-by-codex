...
- **A copy.json** (the `--copy` file of render, pack, judge, copylint, copyjudge and deliver):
  ...
  Each string may also carry `"lang"` (`"bn"`), `"on_image": false`, `"must_exact"` and `"placeholder": true` (an
  agreed placeholder: a value the client sends later, such as a phone number or a link; the render warns instead of
  failing, the judge notes it as P1, and `deliver` lists it to replace before print or posting). `"platform"` also
  takes `print` for posters, flyers, cards and signs. A plain list of strings or
...
| `deliver --design F [--design …] --out D --copy C [--caption T] [--level client\|draft] [--judge --brief B [--kind K] [--allow A] [--goal G] [--runs N]] [--facts J] [--ledger L --recipe R --client C]` | The last gate (`--caption` as given to copyjudge: a file, or the text). `--judge` first runs the design judge on every design and the copy judge on the copy, all at the same time (`--runs` 3 at client level, 1 at draft; `--brand`, `--copy`, `--kind`, `--allow`, `--goal`, `--locale` and `--platform` go to the judges as given), so the final check takes as long as the slowest judge instead of their sum and one model turn; a verdict that already matches its file, brief and settings is reused, and a design whose render failed or copy with lint errors is not sent (the gates stop it anyway). A judge that gives no verdict (a usage limit, a timeout) stops the delivery with Codex's reason. A failed verdict brings the judge's first fixes or notes, so no second call has to read the report. Each design needs a render report without errors that matches the file (hash), a design-judge PASS or PASS_SENIOR on that exact file, and copy that passes copylint and a copy judge that saw this deck with PASS or better (PASS_NATIVE is the target at `--level client`, the default: below it the delivery goes out with a warning and the judge's notes, lines that go beyond the brief first); `--facts` stops on refuted or needs_client claims; `--ledger` rejects a close repeat and records the design. Then the files (a carousel strip brings its slides, a PDF its pages) and the fonts' licences are copied into `D` with `DELIVERY.md` (checks, caption, alt text, notes) and `delivery.json`. Nothing is copied when a gate fails (exit 2); `--dry-run` only runs the gates. | a file already in `D` is moved to `D/archive-<time>/`, never overwritten |
