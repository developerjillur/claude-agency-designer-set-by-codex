#!/usr/bin/env bash
# Install the three skills for Claude Code: link them into ~/.claude/skills (so `git pull` updates them), set up each
...
mkdir -p "$dest"
for skill in codex-imagegen codex-design natural-copy; do
  target="$dest/$skill"
  if [ -L "$target" ]; then
