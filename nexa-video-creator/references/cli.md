...
| `cutout JOB PICTURE...` | a file or a job source id; `--style auto\|color\|bw\|halftone` (auto: people halftone, the rest in colour), `--stroke auto\|none\|#RRGGBB`, `--stroke-width PX`, `--offset DX,DY`, `--shadow soft\|none`, `--largest` (only the biggest subject), `--no-lift`, `--id` | `media/cutouts/ID.png` (transparent, padded, at most 2000 px) and its `.json` record (from, style, people found, a stock licence carried over), added as an `image` source with `alpha` |
